...
db_migration: up_mysql_db ## This is the db migration task.
	...
	@docker run --rm --network="kn-fit-network" -v $(PWD)/migrations:/flyway/sql boxfuse/flyway -url=jdbc:mysql://kn_database -schemas=kn_fit_database -user=root -password=root migrate
	@echo "Done"
